...
(after! ibuffer (annt--evil-collection-init 'ibuffer))

(def-key! [remap list-buffers] #'ibuffer)
#+end_src

/David Wilson/'s tutorial on how to
[[https://www.youtube.com/watch?v=C7ZlNRbWdVI][Organize Your Windows with the Tab Bar in Emacs 27]]
...
#+begin_src emacs-lisp
(defun-hook! annt--tab-bar-setup ()
  emacs-startup-hook
  "Sets up `tab-bar-mode' right after Emacs starts."
  ...
  ;; enable tab history for undo/redo functionality
...
(defun annt--evil-collection-init (modes)
  "Wrapper around `evil-collection-init' which calls mentioned function only
after `evil' has been loaded first.
Argument MODES behaves the same."
  (after! evil (evil-collection-init modes)))
#+end_src
...
#+begin_src emacs-lisp
;; load modus-themes; don't enable yet
...
(after! dashboard
  ...
  (annt--evil-collection-init 'dashboard)
...
(after! dired
  ...
  (annt--evil-collection-init 'dired) ;; vi binds
...
#+begin_src emacs-lisp
(after! prog-mode (straight-use-package 'lsp-mode))
...
#+begin_src emacs-lisp
(after! lisp-mode (straight-use-package 'sly))
...
(unless (executable-find "npm")
 (message
  "npm is not installed (or not found), it may needed for extra functionality"))
#+end_src
...
(after! magit
  ;; vi binds
  (annt--evil-collection-init '((magit magit-repos magit-submodule)
...
(after! helpful
  ;; vi binds
  (annt--evil-collection-init 'helpful))
#+end_src
...
#+begin_src emacs-lisp
(after! prog-mode (straight-use-package 'company)
...
#+begin_src emacs-lisp
(after! prog-mode (straight-use-package 'flycheck))
...
#+begin_src emacs-lisp
(after! org (straight-use-package 'org-tree-slide))
